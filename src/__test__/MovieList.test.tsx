...
// Mock components
jest.mock("../components/Home/MovieCard", () => ({ data, onInfoClick, history }: any) => (
  <div data-testid="movie-card" onClick={() => onInfoClick(data._id)}>
    {data.title}
  </div>
));
...
describe("MovieList Component", () => {
  ...
  it("renders the title", () => {
    render(<MovieList data={mockMovies} title="Test Movies" onInfoClick={handleInfoClick} history={false} />);
    expect(screen.getByText("Test Movies")).toBeInTheDocument();
  });

  it("displays 'No movies found' when there is no data", () => {
    render(<MovieList data={[]} title="Test Movies" onInfoClick={handleInfoClick} history={false} />);
